...
for(let j = 0; j<inputs.length -1; j++) {
    ...
    const {compartment1, compartment2}  = getCompartments(input);

    const duplicate = getDuplicate(compartment1, compartment2);
...
function getDuplicate(compartment1, compartment2) {
    let i = 0;
    let duplicate = '';

    while(compartment2.indexOf(compartment1.charAt(i)) === -1)
        i++;
